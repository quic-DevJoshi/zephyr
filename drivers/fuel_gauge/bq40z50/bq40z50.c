					  ...
					  size_t len)
{
	...
	if (recv_addr != addr || (recv_msg[0] - sizeof(addr)) != len) {
		LOG_ERR("manufacture block access read failed: send cmd: %d\t recv cmd: %d\t recv_len: %d\t requested_len: %d", addr, recv_addr, recv_msg[0], len);
	}
			    ...
			    union fuel_gauge_prop_val val)
{
	...
	case FUEL_GAUGE_SBS_MFR_ACCESS:
		ret = bq40z50_i2c_write(dev, BQ40Z50_MANUFACTURERACCESS, (uint8_t *)&val.sbs_mfr_access_word,
			    ...
			    union fuel_gauge_prop_val *val)
{
	...
	case FUEL_GAUGE_ABSOLUTE_STATE_OF_CHARGE:
		ret = bq40z50_i2c_read(dev, BQ40Z50_ABSOLUTESTATEOFCHARGE, (uint8_t *)&tmp_val, BQ40Z50_LEN_BYTE);
		val->absolute_state_of_charge = tmp_val;
		break;

	case FUEL_GAUGE_RELATIVE_STATE_OF_CHARGE:
		ret = bq40z50_i2c_read(dev, BQ40Z50_RELATIVESTATEOFCHARGE, (uint8_t *)&tmp_val, BQ40Z50_LEN_BYTE);
...
static int bq40z50_get_metadata(const struct device *dev) {
	struct bq40z50_data *data = (struct bq40z50_data*)(dev->data);
	uint8_t firmware_ver[BQ40Z50_FIRMWARE_VERSION_LEN];
	int ret = bq40z50_i2c_read_mfr_blk_acces(dev, BQ40Z50_MAC_CMD_FIRMWARE_VER, (uint8_t*)firmware_ver, BQ40Z50_FIRMWARE_VERSION_LEN);
...
static DEVICE_API(fuel_gauge, bq40z50_driver_api) = 
					{.get_property = &bq40z50_get_prop,
...
#define BQ40Z50_INIT(inst)                                                                         \
	...
	struct bq40z50_data bq40z50_data_##inst = {.major_version = 0x00, .minor_version = 0x00};	\
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, &bq40z50_init, NULL, &bq40z50_data_##inst, &bq40z50_config_##inst,             \
