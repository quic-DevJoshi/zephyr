					  ...
					  size_t len)
{
	...
	// first byte is the length of the data received from the block access and the next two
	// bytes are the cmd.
	...
	if (ret) {
		LOG_ERR("bq40z50_i2c_read_mfr_blk_acces: i2c_burst_write_dt returned %d", ret);
	}

	ret = i2c_burst_read_dt(&cfg->i2c, BQ40Z50_MANUFACTURERBLOCKACCESS, recv_msg, rd_len);
	if (ret) {
		LOG_ERR("bq40z50_i2c_read_mfr_blk_acces: i2c_burst_read_dt returned %d", ret);
	}
			    ...
			    union fuel_gauge_prop_val val)
{
	...
	case FUEL_GAUGE_SBS_MFR_ACCESS:
		ret = bq40z50_i2c_write(dev, BQ40Z50_ATRATE, (uint8_t *)&val.sbs_mfr_access_word,
			    ...
			    union fuel_gauge_prop_val *val)
{
	...
	case FUEL_GAUGE_AVG_CURRENT:
		...
		// convert mA to uA
		val->avg_current = (int)tmp_val * 1000;
		break;
